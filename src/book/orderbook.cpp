...
void Orderbook::calculate_vols() {
    bid_vol_ = std::accumulate(bids_.begin(), std::next(bids_.begin(), std::min(100, static_cast<int>(bids_.size()))), 0ULL,
                               [](uint64_t sum, const auto& pair) { return sum + pair.second->volume_; });

    ask_vol_ = std::accumulate(offers_.begin(), std::next(offers_.begin(), std::min(100, static_cast<int>(offers_.size()))), 0ULL,
                               [](uint64_t sum, const auto& pair) { return sum + pair.second->volume_; });
    //std::cout << "bid_vol_: " << bid_vol_ << ", ask_vol_: " << ask_vol_ << std::endl;
...
float Orderbook::get_best_bid_price() const { return bids_.begin()->first; }

float Orderbook::get_best_ask_price() const { return offers_.begin()->first; }
